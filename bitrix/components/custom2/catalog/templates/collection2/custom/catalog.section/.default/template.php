...
<noindex>
<div style="margin:10px 0 0 0;"><strong>Внимание (!)</strong> Цены на сайте могут отличаться от действующих.<br />
Точную цену товара узнавайте в магазинах или уточняйте по телефону <strong>(495) 777-8-999.</strong>
</div>
</noindex>
<?
if (strlen($arResult["UF_SEC_TOP_DESCR"]) > 0)
{
	?><div style="margin:10px 0 0 0;"><?=$arResult["UF_SEC_TOP_DESCR"]?></div><?
}
?>
...
<table>
		...
		<td class="cat-elem" valign="top">
            ...
            if(is_array($arElement["PREVIEW_PICTURE"]))
			{
	            ...
	            ?>
                <div class="cat_elem"><a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img src="<?=$arElement["PREVIEW_PICTURE"]["SRC"]?>" <?=($image_width_changed)?'width="'.$image_width.'"':"";?> <?=($image_height_changed)?'height="'.$image_height.'"':"";?>  class="cat_elem" alt="<?=$arElement["NAME"]?>"<?=($image_height)?' style="margin:'.round((304-$image_height)/2).'px 0 0 0;"':"";?> /></a></div>
				<?
			}
			 ?>
             <?
			 ...
			 {
				 echo '<div class="cat_elem_price">'.number_format($arElement["DISPLAY_PROPERTIES"]["col_price"]["VALUE"], 0, '.', ' ')." Р</div>";
			 }
			 ?>
			 ...
			 {
				 echo '<div class="cat_elem_brand">'.strip_tags($arElement["DISPLAY_PROPERTIES"]["col_brand"]["DISPLAY_VALUE"]).'</div>';
			 }

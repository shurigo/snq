...
if (isset($arParams["PRICE_SORT"]) && ($arParams["PRICE_SORT"] == "asc" || $arParams["PRICE_SORT"] == "desc"))
{
	...
	{
		$arParams["PRICE_SORT"] = "ASC";
	}
	elseif ($arParams["PRICE_SORT"] == "desc")
	{
		$arParams["PRICE_SORT"] = "DESC";
	}
}
else
{
	$arParams["PRICE_SORT"] = false;
}

if (strlen($arParams["COMPONENT_URL"]) > 0)
{
	$arParams["COMPONENT_URL"] = $arParams["COMPONENT_URL"];
}
else
{
	$arParams["COMPONENT_URL"] = false;
}
...
$arParams["ADD_SECTIONS_CHAIN"] = $arParams["ADD_SECTIONS_CHAIN"]!="N"; //Turn off by default
...
/*************************************************************************
			Work with cache
*************************************************************************/
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $arParams["PRICE_SORT"])))
{
	...
	if ($arParams["PRICE_SORT"] == "ASC" || $arParams["PRICE_SORT"] == "DESC")
	{

...
<?  endif; ?>
    <?
    if (isset($arResult["DISPLAY_PROPERTIES"]["col_add_discount"]["VALUE"]) && ($arResult["DISPLAY_PROPERTIES"]["col_add_discount"]["VALUE"]!=0))  echo '<p style="color:red;">НА ДАННУЮ МОДЕЛЬ ПРЕДУСМОТРЕННА ДОПОЛНИТЕЛЬНАЯ СКИДКА - '.$arResult["DISPLAY_PROPERTIES"]["col_add_discount"]["VALUE"].'%. ПОДРОБНОСТИ У ПРОДАВЦОВ-КОНСУЛЬТАНТОВ.</p>';

      	...
      	if (strlen($arElement["PROPERTY_8"]) > 0) {
            ...
            if ($shop_counter > 1)
            {
                ?>};
                    ...
                    <!-- Ётот блок кода нужно вставить в ту часть страницы, где вы хотите разместить карту (конец) -->
                <?
            }
		}
	}
             ...
             if (is_array($arCity["ITEMS"]) && count($arCity["ITEMS"]) > 0)
            {
                ...
                foreach($arCity["ITEMS"] as $arElement)
                {
                    ...
                    <li>
                        ...
                        <div><b>јдрес: </b><?=$arElement["PROPERTY_63"]?></div>
                        <div><a href="/our_shops/detail.php?id=<?=$arElement['ID']?>" id="<?=$arElement["ID"]?>" target="_blank">ѕодробнее>></a></div>
                        <br />
                    </li>
                    <?
                    $shop_counter++;

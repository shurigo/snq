...
function CheckFields() 
{
	...
	{
		if (ereg("[^A-Za-z_0-9]",$SYMBOLIC_NAME)) 
...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $VOTE_RIGHT>="W" && check_bitrix_sessid())
{
	if (CheckFields())
	{
		...
		if ($ID>0) $DB->Update("b_vote_channel",$arFields,"WHERE ID='".$ID."'",$err_mess.__LINE__);
		else 
		{
...
$sDocTitle = ($ID>0) ? eregi_replace("#ID#","$ID",GetMessage("VOTE_EDIT_RECORD")) : GetMessage("VOTE_NEW_RECORD");

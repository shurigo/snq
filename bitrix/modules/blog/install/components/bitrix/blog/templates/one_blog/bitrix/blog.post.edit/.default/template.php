		...
		if(strlen($_POST["do_upload"])>0)
		{
			...
			<script>
				...
				my_html = '<div class="blog-post-image-item"><div class="blog-post-image-item-border"><?=$arResult["ImageModified"]?></div>' +
					'<div class="blog-post-image-item-input"><input name=IMAGE_ID_title[<?=$arResult["Image"]["ID"]?>] value="<?=htmlspecialchars($arResult["Image"]["TITLE'"])?>"></div>' +
			...
			if(!empty($matches))
			{
				foreach($matches[0] as $key => $value)
				{
					if(strlen($value) > 0)
					{
						...
						?>
						arVideo['<?=$i?>'] = '<?=htmlspecialcharsEx($matches[0][$key])?>';
						arVideoP['<?=$i?>'] = '<?=htmlspecialcharsEx($matches[2][$key])?>';
			...
			<div class="blog-comment-field blog-comment-field-bbcode">
				...
				<div id="edit-post-text" style="display:none;">
					...
					<div class="blog-smiles-line">
						...
						{
							?>
								<img src="/bitrix/images/blog/smile/<?=$arSmiles["IMAGE"]?>" width="<?=$arSmiles["IMAGE_WIDTH"]?>" height="<?=$arSmiles["IMAGE_HEIGHT"]?>" title="<?=$arSmiles["LANG_NAME"]?>" OnClick="emoticon('<?=$arSmiles["TYPE"]?>')" style="cursor:pointer">
							<?
						}
						?>
						<div class="blog-clear-float"></div>

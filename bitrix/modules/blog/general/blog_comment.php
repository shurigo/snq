			...
			if($arGroup["SITE_ID"] == SITE_ID)
			{
				...
				if(!empty($arPost) && $arPost["BLOG_ID"] == $arBlog["ID"] && $arPost["ENABLE_COMMENTS"] == "Y")
				{
					...
					if(strlen($arPathTemplate["PATH_TO_BLOG"])>0)
						$blogURL = "http://".$serverName.CComponentEngine::MakePathFromTemplate($arPathTemplate["PATH_TO_BLOG"], array("blog" => htmlspecialchars($arBlog["URL"]), "user_id" => $arBlog["OWNER_ID"], "group_id" => $arBlog["SOCNET_GROUP_ID"]));
					else
						$blogURL = "http://".$serverName.CBlog::PreparePath($arBlog["URL"], $arGroup["SITE_ID"]);
						
					if(strlen($arPathTemplate["PATH_TO_POST"])>0)
						$url = "http://".$serverName.CComponentEngine::MakePathFromTemplate($arPathTemplate["PATH_TO_POST"], array("blog" => htmlspecialchars($arBlog["URL"]), "post_id"=>$arPost["ID"], "user_id" => $arBlog["OWNER_ID"], "group_id" => $arBlog["SOCNET_GROUP_ID"]));
					else
					...
					elseif ($type == "rss2.0")
					{
						...
						$rssText .= " <channel>\n";
						$rssText .= "	<title>".$blogName."</title>\n";
						$rssText .= "	<guid>".$blogURL."-".$postID."</guid>\n";
						$rssText .= "	<link>".$url."</link>\n";
						$rssText .= "	<language>".$language."</language>\n";
					...
					elseif ($type == "atom.03")
					{
						...
						if(strLen($arPathTemplate["PATH_TO_USER"])>0)
							$authorURLP = "http://".$serverName.CComponentEngine::MakePathFromTemplate($arPathTemplate["PATH_TO_USER"], array("user_id"=>$arPost["AUTHOR_ID"]));
						else
					...
					if($postPerm >= BLOG_PERMS_READ)
					{
						...
						while ($arComments = $dbComments->Fetch())
						{
							...
							{
								if(strLen($arPathTemplate["PATH_TO_USER"])>0)
									$authorURL = "http://".$serverName.CComponentEngine::MakePathFromTemplate($arPathTemplate["PATH_TO_USER"], array("user_id"=>$arComments["AUTHOR_ID"]));
								else
									$authorURL = "http://".$serverName.CBlogUser::PreparePath($arComments["AUTHOR_ID"], $arGroup["SITE_ID"]);
							}

							$text = $parser->convert_to_rss($arComments["POST_TEXT"]);

							$title = GetMessage("BLG_GCM_COMMENT_TITLE", Array("#POST_TITLE#" => $arPost["TITLE"], "#COMMENT_AUTHOR#" => $author));
							$title = str_replace(
								array("&", "<", ">", "\""),
								array("&amp;", "&lt;", "&gt;", "&quot;"),
								$title);

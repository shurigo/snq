...
abstract class Kohana_Captcha
{
	 ...
	 */
	public function image_render($html, $request = NULL)
	{
		// Output html element
		if ($html)
		{
			$attr = Arr::extract(Captcha::$config, array('height', 'width'));
			$attr += array('alt' => 'Captcha', 'class' => 'captcha');
			return HTML::image('captcha/'.Captcha::$config['group'], $attr);
		}
	 ...
	 * @return mixed
	 */
	abstract public function render($html = TRUE, $request = NULL);

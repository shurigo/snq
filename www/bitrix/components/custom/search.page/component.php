	...
	if(strlen($folderPath) > 0)
	{
		...
		if($obSearch->errorno==0)
		{
			...
			while($ar)
			{
				...
				$searchIblockElem = CIBlockElement::GetList(
										array(), 
										array(
				...
				if ($arSearchIblockElem = $searchIblockElem->GetNext())
				{
					if (is_numeric($arSearchIblockElem["PREVIEW_PICTURE"]) && $arSearchIblockElem["PREVIEW_PICTURE"] > 0)
					{
						$resPreviewPic = CFile::GetByID($arSearchIblockElem["PREVIEW_PICTURE"]);
						$arPreviewPic = $resPreviewPic->Fetch();
						$arPreviewPic["SRC"] = CFile::GetPath($arSearchIblockElem["PREVIEW_PICTURE"]);
						$arSearchIblockElem["PREVIEW_PICTURE"] = $arPreviewPic;
					}
					if (is_numeric($arSearchIblockElem["PROPERTY_2_VALUE"]) && $arSearchIblockElem["PROPERTY_2_VALUE"] > 0)
					{
						$resBrand = CIBlockElement::GetByID($arSearchIblockElem["PROPERTY_2_VALUE"]);
						if ($arBrand = $resBrand->GetNext())
						{
							$arSearchIblockElem["BRAND_NAME"] = $arBrand["NAME"];
						}
					}
					//echo "<pre>"; print_r($arSearchIblockElem); echo "</pre>";
					...
					$ar = array_merge($ar, $arSearchIblockElem);
					$arResult["SEARCH"][]=$ar;
				}
				$ar = $obSearch->GetNext();
			}

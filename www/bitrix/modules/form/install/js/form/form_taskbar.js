function BXFormElementsTaskbar()
{
	...
	BXFormElementsTaskbar.prototype.DisplayTree = function ()
	{
		...
		var _arElement = [];
		_arElement['name'] = 'cformelements';
		_arElement['title'] = __arr_messages['FORM_TASKBAR_CFORM'];
		_arElement['tagname'] = 'cformelements';
		_arElement['icon'] = '';
		...
		for (var i=0; i<__arr_input_types.length; i++)
		{
			...
			__arPar[0] = {name:'element_type', value:__arr_input_types[i]}
		
			var __arElement = [];
			__arElement['group'] = 'cformelements';
			__arElement['tagname'] = 'form_question_new';
			__arElement['name'] = __arr_input_types[i];
			__arElement['title'] = __arr_input_types_titles[i];
			__arElement['icon'] = '/bitrix/images/form/visual/form_' + __arr_input_types[i] + '.gif';
			__arElement['isGroup'] = false;
			__arElement['params'] = __arPar;
		...
		{
			var _arElement = [];
			_arElement['name'] = 'cformoutputelements';
			_arElement['title'] = __arr_messages['FORM_TASKBAR_CFORMOUTPUT'];
			_arElement['tagname'] = 'cformoutputelements';
			_arElement['icon'] = '';
			_arElement['isGroup'] = true;
			_arElement['params'] = [];		
			_arElement['childElements'] = [];
			...
			for (var i=0; i<oForm.arQuestions.length; i++)
			{
				...
				{
					var __arPar = [];
					__arPar[0] = {name:'element_index', value:i}
					
					var __arElement = [];
					__arElement['group'] = 'cformoutputelements';
					__arElement['tagname'] = 'form_question';
					__arElement['name'] = oForm.arQuestions[i].FIELD_SID;
					__arElement['title'] = oForm.arQuestions[i].CAPTION;
					__arElement['icon'] = '/bitrix/images/form/visual/form_' + oForm.arQuestions[i].type + '.gif';
					__arElement['isGroup'] = false;
					__arElement['params'] = __arPar;
					_arElement['childElements'].push(__arElement); 
				}
		...
		var _arElement = [];
		_arElement['name'] = 'cformoutputapimethods';
		_arElement['title'] = __arr_messages['FORM_TASKBAR_API'];
		_arElement['tagname'] = 'cformoutputapimethods';
		_arElement['icon'] = '';
		_arElement['isGroup'] = true;
		_arElement['params'] = [];		
		_arElement['childElements'] = [];
		...
		{
			var __arPar = [];
			  __arPar[0] = {name:'METHOD', value:__arr_api_methods[i]}
			
			var __arElement = [];
			__arElement['group'] = 'cformoutputapimethods';
			__arElement['tagname'] = 'form_api';
			__arElement['name'] = __arr_api_methods[i];
			__arElement['title'] = __arr_api_methods_title[i];
			__arElement['icon'] = '/bitrix/images/form/visual/form_api_' + __arr_api_methods[i].toLowerCase() + '.gif';
			__arElement['isGroup'] = false;
			__arElement['params'] = __arPar;
	...
	BXFormElementsTaskbar.prototype.ShowAPIProperties = function(_bNew, _pTaskbar, _pElement)
	{
		...
		var __APIMethodName = arParams[0].value;

		var __saveParams = function(e)
		{
			__element_index = arParams[1].value;
			...
			if (__arr_api_methods_params[__APIMethodName].length > 0)
			{
				...
				{	
					...
					{
						arParams[2].value[__i].value = __obj.checked ? "Y" : "N";
					}
					else if (__obj.tagName == "SELECT")
					{
						...
						arParams[2].value[__i].value = __obj_self && __obj_self.value != '' ? __obj_self.value : __obj.value;
					}
					else
					{
						arParams[2].value[__i].value = __obj.value;
					}
				}
			}
			
			oForm.arMethods[__element_index].params = arParams[2].value;
		}
		...
		if (arParams[1])
		{
			if (oForm.arMethods[arParams[1].value])
			{
				arParams[2] = {name:'PARAMS', value:oForm.arMethods[arParams[1].value].params}
			}
		}
		
		//_div.appendChild(document.createTextNode(returnProperties(arParams)))
		//_div.appendChild(document.createElement('BR'));

		var __tn = document.createElement('B');
		var __method_caption = arParams[0].value;
		...
		if (arParams.length < 3 || arParams[2].value.length <= 0)
		{
		...
		else 
		{
			...
			for(var i=0; i<__arr_api_methods_params[__APIMethodName].length; i++)
			{
				...
				param_value = oForm.getMethodParamValue(arParams[1].value, param_name);
				...
				switch (param_name)
				{
					case 'CSS_STYLE':
						...
						/*
						var style_include = document.getElementsByTagName("STYLE")[0];
						if (!style_include) 
						{
							style_include = document.createElement("STYLE");
							_div.appendChild(style_include);
						}
						
						style_include.appendChild(document.createTextNode(GLOBAL_pMainObj['FORM_TEMPLATE'].oStyles.sStyles));
						*/
						
						//GLOBAL_pMainObj['FORM_TEMPLATE'].oStyles.SetToDocument(document);
	...
	BXFormElementsTaskbar.prototype.ShowProperties = function(_bNew, _pTaskbar, _pElement)
	{
		...
		var arParams = oBXEditorUtils.getCustomNodeParams(_pElement);
		var element_index = arParams[0].value;
		...
		{	
			...
			if (_i == "structure")
			{
				...
				{
					oBXEditorUtils.BXRemoveAllChild(_input);
				}
			...
			else
			{
				...
				if (__pt_cell)
				{
					oBXEditorUtils.BXRemoveAllChild(__pt_cell);
					__pt_cell.appendChild(_input);
				}
			}
		}
	...
	BXFormElementsTaskbar.prototype.UnParseElement = function(node)
	{
		if (node.arAttributes["__bxtagname"] == 'form_question')
		{
			var __arNodeParams = oBXEditorUtils.getCustomNodeParams(node)
			var element_index = __arNodeParams[0].value;
			
			if (null != element_index && null != oForm.arQuestions[element_index].FIELD_SID)
			{
		...
		if (node.arAttributes["__bxtagname"] == 'form_api')
		{
			var __arNodeParams = oBXEditorUtils.getCustomNodeParams(node);
			
			if (null != __arNodeParams[0]["value"])
			{
				var _param = '';			
				
				if (__arNodeParams[1] && __arNodeParams[1]["name"] == "INDEX" && oForm.arMethods[__arNodeParams[1]["value"]]) 
				{
					if (oForm.arMethods[__arNodeParams[1]["value"]].params)
					{
						__params = oForm.arMethods[__arNodeParams[1]["value"]].params;
				...
				{
					_param = null != __arNodeParams[1] ? '"' + __arNodeParams[1]["value"] + '"' : '';
				}
				
				return '<' + '?=$FORM->' + __arNodeParams[0]["value"] + '(' + _param + ')?' + '>';
			}
	...
	BXFormElementsTaskbar.prototype.OnElementDragEnd = function(oEl)
	{
		...
		var __bxtagname = oEl.getAttribute('__bxtagname');
		var __arPar = oBXEditorUtils.getCustomNodeParams(oEl);
		
		if (__bxtagname == 'form_question')
		{
			var element_index = __arPar[0].value;
			var element_type = oForm.arQuestions[element_index].type;
		...
		else if (__bxtagname == 'form_question_new')
		{
			var element_type = __arPar[0].value;
			var __arStructure = [];
			var __csort = 0;
			var __imax = 1;
			
			oEl.setAttribute('__bxtagname', 'form_question');
			oEl.setAttribute('src', '/bitrix/images/form/visual/form_edit_' + element_type + '.gif');
			oEl.removeAttribute("height");
			oEl.removeAttribute("width");
			oEl.style.width = null;
			...
			__arPar = null;
			__arPar = [{name:'element_index', value:element_index}];
			
			oBXEditorUtils.setCustomNodeParams(oEl, __arPar);
			
			return;
		}
		else if (__bxtagname == 'form_api')
		{
			var element_type = __arPar[0].value.toLowerCase();
			...
			__ar_params = [];
			for (var j = 0; j<__arr_api_methods_params[__arPar[0].value].length; j++)
			{
				var param = __arr_api_methods_params[__arPar[0].value][j];
				__ar_params[j] = {name:param, value: ''};
			}
			
			var _index = oForm.addMethodInstance(__arPar[0].value, __ar_params);
			
			__arPar[1] = {name:'INDEX', value:_index};

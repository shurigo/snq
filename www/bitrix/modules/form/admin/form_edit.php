				...
				foreach ($arrFS as $arQuestion)
				{
					...
					if ($QID)
					{
						foreach ($arQuestion["structure"] as $arAnswer)
						{
							...
							if (in_array($arAnswer["FIELD_TYPE"], array("dropdown", "multiselect")) && $arAnswer["DEFAULT"] == "Y") $arAnswer["FIELD_PARAM"] = "SELECTED";
							if (in_array($arAnswer["FIELD_TYPE"], array("checkbox", "radio")) && $arAnswer["DEFAULT"] == "Y") $arAnswer["FIELD_PARAM"] = "CHECKED";
...
else
{
  ...
  arrInputObjects[<?=$i++?>] = new CFormAnswer(
  	'<?=$FIELD_SID?>',
  	'<?=CUtil::JSEscape(htmlspecialchars($FORM->__admin_ShowInputCaption($FIELD_SID, "tablebodytext", true)))?><?=($arQuestion['ACTIVE'] == 'N' ? ' (<i>'.GetMessage('F_QUESTION_INACTIVE').'</i>)' : '')?>',
...
<?
		...
		CFileMan::ShowHTMLEditControl(
			...
			array(
				...
				"bWithoutPHP"=>false,
				"arToolbars"=>Array("base_form_tpl", "standart", "style", "formating", "source", "template", "table"),
				"arTaskbars"=>Array("BXPropertiesTaskbar", "BXFormElementsTaskbar"),
				"sBackUrl" => "",
				"fullscreen" => false,
				'width' => defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 ? '100%' : '750',
				'height' => defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1 ? '450' : '450',
			)
		);

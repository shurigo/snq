...
// переменная с уникальным идентификатором таблицы
...
if(($arID = $lAdmin->GroupAction()) && /*$FORM_RIGHT>="W"*/ $F_RIGHT >= 20 && check_bitrix_sessid())
{
	...
	if ($_SERVER['REQUEST_METHOD'] == 'GET')
		LocalRedirect('/bitrix/admin/form_result_list.php?lang='.LANGUAGE_ID.'&WEB_FORM_ID='.$WEB_FORM_ID);
}
...
if (strlen($TABLE_RESULT_TEMPLATE)>0 && file_exists($_SERVER["DOCUMENT_ROOT"].$TABLE_RESULT_TEMPLATE)) // используем шаблон пользователя
{
	$custom_table=true;
}
else
{
	...
	#	print "<pre>"; print_r($arValues); print "</pre><hr>";
	...
	while($arRes = $result->NavNext(true, "f_"))
	{
		...
		foreach ($arrAnswers[$f_ID] as $arFieldValues)
		{
			if ($f_USER_ID>0)
			{
				...
				$txt = "[<a title='".GetMessage("FORM_EDIT_USER")."' href=\"user_edit.php?lang=".LANGUAGE_ID."&ID=".$f_USER_ID."\">$f_USER_ID</a>] ($f_LOGIN) $f_USER_NAME";
			...
			{
	#			print "<pre>";print_r($arrAnswers);print "</pre><hr>";
		...
		if ($F_RIGHT>=20 || ($F_RIGHT>=15 && $USER_ID==$f_USER_ID))
		{
			...
			if (in_array("DELETE",$arrRESULT_PERMISSION))
				$arActions[] = array("ICON"=>"delete", "TITLE"=>GetMessage("FORM_DELETE_ALT"), "TEXT"=>GetMessage("FORM_DELETE"), "ACTION"=>$lAdmin->ActionRedirect("javascript:if(confirm('".GetMessage("FORM_CONFIRM_DELETE")."')) window.location='?lang=".LANGUAGE_ID."&WEB_FORM_ID=$WEB_FORM_ID&WEB_FORM_NAME=$WEB_FORM_NAME&action=delete&ID=$f_ID&".bitrix_sessid_get()."'"));
		}
...
// контекстное меню на добавление, переход к спискам и т.п.
$aMenu[] = array(

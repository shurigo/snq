...
function GetVoteDataByID($VOTE_ID, &$arChannel, &$arVote, &$arQuestions, &$arAnswers, &$arDropDown, &$arMultiSelect, &$arGroupAnswers, $getGroupAnswers)
{
	...
	if (!empty($GLOBALS["VOTE_CACHE_VOTING"][$VOTE_ID]["V"])):
		...
		if (!($db_res && $arVote = $db_res->GetNext(true, false))):
			return false;
		endif;
	...
	if (!empty($GLOBALS["VOTE_CACHE_VOTING"][$VOTE_ID]["QA"]))
	{
		...
		{
			foreach($arAnswers as $arQ):
				foreach ($arQ as $ar):
					if ($ar["FIELD_TYPE"] == "4" || $ar["FIELD_TYPE"] == "5"):
						$g = CVoteAnswer::GetGroupAnswers($ar["ID"]);
						while ($gr=$g->GetNext(true,false)) $arGroupAnswers[$ar["ID"]][] = $gr;
					endif;
				endforeach;
			endforeach;
	...
	else
	{
		...
		while ($res = $db_res->GetNext(true,false)) 
		{
			...
			{
				$arAnswers[$res["ID"]][] = $wr;
				if ($getGroupAnswers=="Y" && ($wr["FIELD_TYPE"]=="4" or $wr["FIELD_TYPE"]=="5")):
					$g = CVoteAnswer::GetGroupAnswers($wr["ID"]);
					while ($gr=$g->GetNext(true,false)) $arGroupAnswers[$wr["ID"]][] = $gr;
				endif;
			}
		}
		...
		foreach ($arQuestions as $key => $arQ)
		{
			...
			if (count($arDropReference) > 0)
				$arDropDown[$QUESTION_ID] = array("reference"=>$arDropReference, "reference_id"=>$arDropReferenceID);
			if (count($arMultiReference)>0)
				$arMultiSelect[$QUESTION_ID] = array("reference"=>$arMultiReference, "reference_id"=>$arMultiReferenceID);
		}
		reset($arChannel);
		reset($arVote);
		reset($arQuestions);

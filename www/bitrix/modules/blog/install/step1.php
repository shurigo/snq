...
<?=bitrix_sessid_post()?>
	...
	<tr>
		...
		<?foreach($arSites as $fSite):?>
			<td><input type="checkbox" name="install_public_<?=$fSite["SITE_ID"]?>" value="Y" onclick="ChangeInstallPublic(this.checked, '<?=$fSite["SITE_ID"]?>')"></td>
		<?endforeach;?>
	</tr>
	<tr>
	...
	<!--
	<?foreach($arSites as $fSite):?>
		ChangeInstallPublic(false, '<?=$fSite["SITE_ID"];?>');
	<?endforeach;?>
	//-->
	</script>
	<br>

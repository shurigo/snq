$(window).load(function(){
  ...
  var gal = $('div.slider1').gallery({
    duration: 800,
    autoRotation: 5000,
    switcher: 'ul.nav > li',
    listOfSlides: 'div.hold > ul > li',
    onChange: function(){
      $('div.slider1 div.line > div').css({left: 100/5*gal.active + '%'});
    }
  });
  $('div.slider1 div.line > div').css({left: 100/5*gal.active + '%'});
  $('div.slider2').gallery({
...
function reloadPage(){
  ...
  $.ajax({
    ...
    error: function(){
      $(window).alert('Server is unavailable. Refresh the page within 15 seconds.!');
    },
  });
}
...
Gallery.prototype = {
  ...
  init: function (context, options) {
    ...
    if (!this.options.direction) this.installDirections(this.list.parent().width());
      ...
      if (this.options.autoHeight) this.list.parent().css({
        height: this.list.children().eq(this.active).outerHeight()
      });
    } else {
      this.rew = this.count;
      this.list.css({
        opacity: 0
      }).removeClass(this.options.active).eq(this.active).addClass(this.options.active).css({
        opacity: 1
      }).css('opacity', 'auto');
      this.switcher.removeClass(this.options.active).eq(this.active).addClass(this.options.active);
      if (this.options.autoHeight) this.list.parent().css({
  ...
  initDisableBtn: function () {
    ...
    if (this.active === 0 || this.count + 1 === this.wrapHolderW - 1) {this.prevBtn.addClass('prev-' + this.options.disableBtn);}
    if (this.active == 0 && this.count + 1 == 1 || this.count + 1 <= this.wrapHolderW - 1) {this.nextBtn.addClass('next-' + this.options.disableBtn);}
  ...
  installDirections: function (temp) {
    ...
    if (this.wrapHolderW == 0) this.wrapHolderW = 1;
  },
  fadeElement: function () {
    if (jQuery.browser.msie && this.options.IE) {
      ...
      this.list.removeClass(this.options.active).eq(this.active).addClass(this.options.active).css({
        opacity: 'auto'
      });
    } else {
      ...
      this.list.removeClass(this.options.active).eq(this.active).addClass(this.options.active).animate({
        opacity: 1
      }, {
        queue: false,
        duration: this.options.duration,
    ...
    if (this.options.switcher) this.switcher.removeClass(this.options.active).eq(this.active).addClass(this.options.active);
    this.last = this.active;
  },
  scrollElement: function ($this) {
    if (!$this.options.infinite) $this.list.animate($this.dirAnimate(), {
      queue: false,
      easing: $this.options.easing,
      duration: $this.options.duration
    });
    else $this.list.animate($this.dirAnimate(), $this.options.duration, $this.options.easing, function () {
      $this.flag = true
    }); if (this.options.autoHeight) this.list.parent().animate({
      height: this.list.children().eq(this.active).outerHeight()
    }, {
    ...
    if ($this.options.switcher) $this.switcher.removeClass($this.options.active).eq($this.active / $this.options.slideElement).addClass($this.options.active);
  },
  ...
  initEventSwitcher: function ($this, el) {
    el.bind($this.options.event, function () {
      ...
      else $this.fadeElement(); if ($this.options.autoRotation) $this.runTimer($this);
      $this.options.onChange();
  ...
  reCalcWidth: function () {
    ...
    if (!this.options.direction) this.installDirections(this.list.parent().width());
      ...
      this.woh = list.children().outerHeight(true);
      this.installDirections(this.list.parent().height());
    }
    this.rew = this.count - this.wrapHolderW + 1;
    this.list.css({

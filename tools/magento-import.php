  ...
  class MagentoImport {
    ...
    private $magento_fields_product = array(
      ...
      'Мужчины' => array(
		...
		private function ConvertCountry($input) {
			$country = mb_convert_case($input, MB_CASE_TITLE, 'UTF-8'); 
			if($country == 'Тайланд') {
				$country = 'Таиланд';
		...
		function GetImagesByIdfmc($idfmc, $img_dir, $hdr) {
			$img_idx = 0;
			$ret = array();
			foreach(glob($img_dir.'/'.$idfmc.'*.jpg') as $filename) {
				++$img_idx;
				//$buf = array_fill(0, count($hdr), null);
				$buf = array();
				$buf[$hdr['image']] = "/".basename($filename);
				$buf[$hdr['_media_image']] = "/".basename($filename);
				$buf[$hdr['small_image']] = "/".basename($filename);
				$buf[$hdr['thumbnail']] = "/".basename($filename);
				$buf[$hdr['_media_attribute_id']] = '88';
				$buf[$hdr['_media_lable']] = $img_idx == 2 ? 'back' : 'image'.$img_idx;
				$buf[$hdr['_media_position']] = $img_idx;
				$buf[$hdr['_media_is_disabled']] = '0';
				$buf[$hdr['_attribute_set']] = 'Default';
				ksort($buf);
				$ret[] = $buf;
			}
			return $ret;
...
//					++$line; // debug
					...
					$buf[$hdr_magento['country_of_manufacture']] = $this->ConvertCountry($data[$hdr_snowqueen['VARTCOUNTRYRUS']]);
          ...
          $buf[$hdr_magento['country_of_design']] = $this->ConvertCountry($data[$hdr_snowqueen['VDESIGN']]);
          $buf[$hdr_magento['produced_from']] = $data[$hdr_snowqueen['VSARTFACTURE']];
          $buf[$hdr_magento['product_care']] = $data[$hdr_snowqueen['VCARE']];
					...
					if(!isset($this->product_category_map[$category][$src_category])) {
						print "ERROR: Failed to map category:\t$src_category\n";
						print $data_str;
					} else {
						$category2 = $this->product_category_map[$category][$src_category];
					}
...
//					$this->logger->error("Failed to open file for reading: $this->file_name");
				...
				foreach($data_new as $idfmc => $data) {
					if(count($data) > 1) { // Configurable
						...
						$images = $this->GetImagesByIdfmc($idfmc, $image_dir, $hdr_magento);
